...
const postWriteAll = async (formObj:Record<string,any>,formEl: FormInstance | undefined) :Promise<void> => {
  ...
  if(props.essayFrom._id) {
    try {
      ...
      const res = await patchWriteEdit(formObj)
  ...
  // 创作逻辑
  try {
    const res = await postWriteInfo(formObj)
    if(res.data.code === '3000') {
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      hooks.message('请完善您的内容，谢谢','error')
    }
  })
}
